...
Cada elemento del sitio público mantiene continuidad con una capacidad del producto. Las páginas de **Solutions** funcionan como rutas comerciales para empresas interesadas; no reemplazan los roles funcionales del ecosistema Nexa. Los roles definitivos del producto son **Sales**, **Logistics / Operations**, **Company Owner** y **B2B Buyer**. La continuidad entre Landing Page y producto se entiende dentro del modelo **multi-tenant SaaS B2B**: la empresa interesada solicita una demostración o accede al login, y los usuarios autenticados ingresan a vistas filtradas por rol, tenant/workspace y alcance de permisos.
...
|---|---|---|---|
| Hero / Pitch message | Presentar el problema operativo y la promesa central de Nexa en el primer contacto visual | Introduce el flujo transversal: B2B Buyer solicita, Sales valida, Logistics / Operations ejecuta y Company Owner administra el workspace | Landing Page |
...
| CTA `Log in` / `Ingresar` | Permitir acceso a usuarios autenticados | Entrada al sistema autenticado con acceso filtrado según rol, tenant/workspace y scope de permisos | `#/auth/login` |
...
| Platform | Explicar capacidades de catálogo, pedidos, inventario, despacho y trazabilidad | Conecta la propuesta pública con dashboards y módulos internos para Sales, Logistics / Operations, Company Owner y B2B Buyer | `/ops/commercial/dashboard`, `/ops/operations/dashboard`, `/portal/home` |
...
| Cold Storage Operators | Comunicar control de almacenamiento, trazabilidad y operación refrigerada | Relaciona Logistics / Operations con inventario, lotes, control operativo y trazabilidad | `/ops/operations/inventory-control`, `/ops/operations/operational-analytics` |
| Pricing | Presentar alternativas comerciales, planes o contacto para iniciar conversación con el equipo | Conecta la evaluación comercial con demo, contacto y posterior administración de plan por Company Owner | Landing Page / Company contact |
...
| Continuidad con producto | Mensajes y rutas conectan con demo, login, catálogo, solicitudes, pedidos, inventario, administración de workspace y portal comprador | Evita que la Landing Page sea un artefacto aislado y mantiene continuidad con las vistas autenticadas por rol, tenant/workspace y scope |
