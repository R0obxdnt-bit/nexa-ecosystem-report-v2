...
Cada elemento del sitio público mantiene continuidad con una capacidad del producto. Las páginas de **Solutions** funcionan como rutas comerciales para empresas interesadas; no reemplazan los segmentos del producto dentro del ecosistema Nexa. La continuidad con el producto se organiza a partir de los segmentos S1 — Commercial Coordination, S2 — Operations / Account Owner y S3 — B2B Buyer Portal. La continuidad entre Landing Page y producto se entiende dentro del modelo **multi-tenant SaaS B2B**: la empresa interesada solicita una demostración o accede al login, y los usuarios autenticados ingresan a vistas filtradas por perfil, tenant/workspace y alcance de permisos.
...
|---|---|---|---|
| Hero / Pitch message | Presentar el problema operativo y la promesa central de Nexa en el primer contacto visual | Introduce el flujo transversal: el S3 solicita, el S1 valida, el S2 ejecuta la operación y el account ownership del S2 administra el tenant/workspace | Landing Page |
| CTA `Request a demo` / `Solicitar una demostración` | Convertir el interés del visitante en contacto comercial | Conecta al visitante o empresa interesada con el contacto comercial, revisión de operación y solicitud de demostración | Landing Page / Company contact |
| CTA `Log in` / `Ingresar` | Permitir acceso a usuarios autenticados | Entrada al sistema autenticado con acceso filtrado según perfil, tenant/workspace y scope de permisos | `#/auth/login` |
...
| Platform | Explicar capacidades de catálogo, pedidos, inventario, despacho y trazabilidad | Conecta la propuesta pública con dashboards y módulos internos para S1, S2 (incluyendo account ownership) y S3 | `/ops/commercial/dashboard`, `/ops/operations/dashboard`, `/portal/home` |
...
| Importers & Wholesalers | Comunicar valor para operaciones con abastecimiento, stock y lotes | Relaciona validación comercial, disponibilidad, inventario y gestión operativa dentro del tenant/workspace | `/ops/commercial/purchase-requests`, `/ops/operations/inventory-control` |
| Distributors | Comunicar el flujo principal de pedido B2B, disponibilidad, despacho y evidencia | Relaciona Buyer Portal, solicitudes de compra, órdenes de compra, validación comercial y órdenes de despacho | `/portal/product-catalog`, `/ops/commercial/purchase-requests`, `/ops/operations/dispatch-orders` |
| Cold Storage Operators | Comunicar control de almacenamiento, trazabilidad y operación refrigerada | Relaciona el S2 con inventario, lotes, control operativo y trazabilidad | `/ops/operations/inventory-control`, `/ops/operations/operational-analytics` |
| Pricing | Presentar alternativas comerciales, planes o contacto para iniciar conversación con el equipo | Conecta la evaluación comercial con demo, contacto y posterior administración de plan por el account ownership del S2 | Landing Page / Company contact |
...
| Continuidad con producto | Mensajes y rutas conectan con demo, login, catálogo, solicitudes, pedidos, inventario, administración de workspace y portal comprador | Evita que la Landing Page sea un artefacto aislado y mantiene continuidad con las vistas autenticadas por perfil, tenant/workspace y scope |
