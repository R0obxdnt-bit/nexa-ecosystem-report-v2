...
Nexa funciona bajo un modelo Multi tenant SaaS orientado a empresas B2B de cadena de frío. Dentro de este modelo, Segmento 1 y Segmento 2 son usuarios internos de la empresa contratante, mientras que Segmento 3 corresponde al comprador externo habilitado para interactuar con el portal de compra. Las responsabilidades administrativas de empresa, accesos, planes y configuración son asumidas por Segmento 2 como **Operations / Account Owner**.
...
| Usuarios internos | Segmento 1 y Segmento 2. Trabajan dentro del tenant de la empresa contratante y participan en el flujo comercial-operativo. |
...
| Account ownership | Responsabilidad asumida por Segmento 2 para administrar empresa, accesos, suscripción, operación e información crítica del tenant. |
...
| **Segmento 3: B2B Buyer Portal** | Cliente comprador B2B: restaurante, supermercado, retail, comprador mayorista, comprador minorista. | Consulta catálogo, arma solicitudes, revisa estado, conversa con Segmento 1, descarga documentos y sigue el despacho de sus pedidos. | Product Catalog, Request Builder, My Requests, My Orders, Business Documents, Premium Catalog, Assistant Preview, Buyer Profile. |
...
*Flujo base actualizado de Nexa*

| Paso | Actor | Flujo | Descripción |
|---|---|---|---|
| 1 | Segmento 3 / Segmento 1 | Entrada de pedido | El comprador arma una solicitud desde el portal o Segmento 1 registra un pedido recibido por WhatsApp, llamada, Excel u otro canal tradicional. |
| 2 | Segmento 1 | Validación comercial | Se revisa RUC/cliente, crédito, stock disponible, observaciones, dirección y documentos requeridos. |
| 3 | Segmento 1 | Conversión a purchase order | La solicitud validada se convierte en pedido confirmado, dejando trazabilidad del origen. |
| 4 | Segmento 2 | Reserva e inventario | Operaciones valida lotes, FEFO, disponibilidad real, temperatura y prioridad de despacho. |
| 5 | Segmento 2 | Dispatch order | Se prepara ruta, responsable, estado, evidencias y proof of delivery. |
| 6 | Segmento 1 / Segmento 2 | Business documents | Se gestionan factura referencial, guía, XML, CDR, POD y tareas manuales de portales externos. |
| 7 | Segmento 3 | Seguimiento y cierre | El comprador ve tracking, estado, documentos visibles, comentarios y alertas del pedido. |
...
```mermaid
flowchart TD
    P1["1. Entrada de pedido\nSegmento 3 solicita desde el portal o Segmento 1 registra un pedido manual"]
    P2["2. Validación comercial\nSegmento 1 revisa cliente, crédito, stock, dirección y documentos"]
    P3["3. Conversión a purchase order\nSegmento 1 convierte la solicitud validada en pedido confirmado"]
    P4["4. Reserva e inventario\nSegmento 2 valida lotes, FEFO, disponibilidad real y prioridad"]
    P5["5. Dispatch order\nSegmento 2 prepara ruta, responsable, estado y evidencias"]
    P6["6. Business documents\nSegmento 1/Segmento 2 gestionan guía, factura referencial, XML, CDR y POD"]
    P7["7. Seguimiento y cierre\nSegmento 3 revisa tracking, documentos, comentarios y alertas"]
...
En Nexa, el Segmento 2 conecta directamente con módulos como **Inventory Control**, **Dispatch Orders**, **Proof of Delivery**, **Operational Analytics**, **Promotions**, **Customer Portals** y **Company Administration**.
...
| Coordinación con varias áreas. | Los estados del pedido deben ser compartidos y entendibles para Segmento 1 y Segmento 2. |
| Control de evidencias y POD. | Las evidencias deben asociarse al pedido y al despacho correspondiente. |
| Administración de empresa, accesos y configuración. | El Segmento 2 debe contar con funciones de company administration dentro del tenant. |
...
En Nexa, el Segmento 3 conecta directamente con módulos como **Product Catalog**, **Request Builder**, **My Requests**, **My Orders**, **Business Documents**, **Premium Catalog**, **Assistant Preview** y **Buyer Profile**.
...
*Impacto de los segmentos en el MVP y métricas de validación*

| Segmento objetivo | Función dentro del MVP | Funcionalidades relacionadas | Métricas de validación sugeridas |
|---|---|---|---|
| Segmento 1 | Validar la captura estructurada de solicitudes, la validación comercial y la conversión a pedidos confirmados. | Purchase Requests, Purchase Orders, Manual Order Entry, B2B Clients, Business Documents y Product Catalog visible. | Porcentaje de solicitudes completas, tiempo promedio de validación, número de pasos manuales entre captura y pedido confirmado, y cantidad de aclaraciones antes de confirmar. |
| Segmento 2 | Validar el control operativo del tenant, la disponibilidad real, la preparación, el despacho, las evidencias y la administración interna. | Inventory Control, Dispatch Orders, Proof of Delivery, Operational Analytics, Promotions, Customer Portals y Company Administration. | Porcentaje de pedidos validados sin ajuste manual, pedidos con estado actualizado, despachos con evidencia registrada, incidencias por pedido y configuraciones críticas administradas desde la plataforma. |
| Segmento 3 | Validar la utilidad del portal para consulta, solicitud, seguimiento, documentos y continuidad de abastecimiento. | Product Catalog, Request Builder, My Requests, My Orders, Business Documents, Premium Catalog, Assistant Preview y Buyer Profile. | Tiempo entre solicitud y confirmación, productos consultados antes de solicitar, pedidos digitales que no requieren llamada adicional, consultas de estado desde el portal y documentos revisados por pedido. |
...
*Conclusiones de trazabilidad por segmento*

| Segmento | Conclusión |
|---|---|
| Segmento 1 | Justifica las épicas e historias relacionadas con solicitudes, validación comercial, clientes B2B, documentos y conversión a pedidos confirmados. |
| Segmento 2 | Justifica las épicas e historias relacionadas con inventario, lotes, FEFO, despacho, evidencias, administración de empresa, accesos y control del tenant. |
| Segmento 3 | Justifica las épicas e historias relacionadas con catálogo, portal comprador, creación de solicitudes, seguimiento de pedidos, consulta de documentos y comunicación con la empresa proveedora. |
